...
library(readxl) # this is to read in .xlsx data
...
I'll begin by first loading in the raw data obtained from Sports Reference and then imported and formatted in Excel. To read this Excel File, I am using the `read_excel()` function from the `readxl` library
```{r get-ku-data}
## obtain the raw data from which we will obtain the points
ku_raw_data <- read_excel("../data/kumbb23.xlsx")
...
Overall, this was a fun project to work on. I actually tried to import the data directly from a link and use `rvest` library to unravel it but I encountered empty values and duplicate column names that I just could not work around. This project introduced me to how to import csv text to an Excel file and serve as my introduction to `readxl` as well. I also was able to use the `kableExtra` library to format my outputted summary data which I now know I will make extensive use of. 
