...
One assumption that will underline this report is the fact the assumption that any and all free throws, for the same stage of the game $S_n$ is the same. Consider a shooting regular foul $L$ on a two-point shot that occurred at the very beginning of the first quarter ($n = 1.1$). Similarly, consider the exact same foul for the last minutes of the fourth quarter ($n = 4.4$). Foul shot $F$ has a success probability $p = P(F)$ Across both cases $n$ is different so $P(F)$ may be different. But if $L$ occurs under the same $n$, $P(F)$ is the same. The only thing differentiating the probability of $F$ is $S_n$.
...
$$
F \in S_n \mid n = \{1.1, 1.2, 1.3, 1.4, 1.5, 2.1, ..., 2.5, 3.1, ..., 4.5\}
$$
...
Similarly, we will do the same for game point differential $D$. We use the $S_n$ assumption above to make this assumption. The assumption is that for any and all free throws, for the categorically same $D$ occurring in $S_n$ , foul shot $F$ with probability $P(F)$ is the same.

Consider a shooting regular foul $L$ on a two-point shot that occurred at the very beginning of the first quarter ($n = 1.1$) with the ***home*** team up by 10 ($D = 10$). Similarly, consider the exact same foul just moments later ($n=1.1$) but now the ***visiting*** team is up by 5 ($D = -5$). Foul shot $F \in L$ has a success probability $p = P(F)$. Across both cases $D$ is different so $P(F)$ may be different. But if $L$ occurs under the same $D$, $P(F)$ is the same. The only thing differentiating $P(F)$ is $D$.
...
$$
D \in S_n \mid n = \{1.1, 1.2, 1.3, 1.4, 1.5, 2.1, ..., 2.5, 3.1, ..., 4.5\}
$$
...
Once we find $m$, we will then assign $m$ to its respective player $p$ to become $m_p$. We will do this for all $p$ so that we can accomplish a dynamic function $f$ that, when passed in $p$, $S_n$ and $D$, returns $m_p$



### Report Intentions

The rest of this report will be divided into three parts of data analysis: part one will focus on finding $m$ for some $p$ and part two will aim to build $f$. Finally, part three aims to show how $f$ can be used in as many informational contexts as possible.  
...
free_throws_2022 <- pbp_2022_data %>% 
  ...
  mutate(
    ...
    team_score = as.numeric(
      str_extract(de, regex(score_regex, TRUE))), 
...
##responsible for classifying certain stage of the game in period.int format
  ...
  select(GAME_ID, home_team, away_team, stage_of_game, de, hs, vs, player_shooting, player_team, pts, pt_difference, pt_difference_cat)
...
As seen above, few new variables were introduced in this cleaner set: 
  ...
  - `home_team` the home team of this match identified by the `GAME_ID` and is associated with `de`
  
  - `away_team` the away team of this match identified by the `GAME_ID` and is associated with `de` 
  
  - `player_shooting` the player shooting the free throw 
  
  - `player_team` the team of the player shooting the free throw
  
  - `pt_difference` the difference between `hs` and `vs`, positive indicates `home_team` score advantage 
...
clean_player_data <- raw_player_data %>%
  ...
  filter(fta != 0) %>%
  filter(fga != 0) %>%
  select(team, player, gms_played, min_per_gm, usage_pct, fgm, fga, fg_pct, eff_fg_pct, ftm, fta, ft_pct, ft_rate)
