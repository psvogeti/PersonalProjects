...
ft_model_complete <- predict.glm(ft_model_train, ft_complete, type='response')
...
ft_complete <- ft_complete %>%
  mutate(model_prob = 0)
for(i in 1:NUM_PLAYERS) {
  ft_complete$model_prob[i] = ft_model_complete[[i]]
}
```
...
ft_dummy <- ft_complete

ft_dummy <- ft_dummy %>%
  group_by(stage_of_game, pt_difference_cat) %>%
  mutate(ft_pct_by_group = mean(pts))
```
