...
Here, we are going to find a basis for $m_p$ using the context of one particular player $p = \text{LeBron James}$. Why him? LeBron has been in the conversation for the greatest player of all time in the NBA. He has been mentioned in countless in-game situations and hypotheticals, making him a robust choice to see what we need to consider when building $m_p$

In order to reflect our table outlined in the [Methodology] section, we need to get $n$. Since $n$ is merely the null model with its intercept term representing the season average free throw percentage, this is a quick fetch.
...
Before we build $m$, we need to join LeBron's statline to his free throw data. This will allow us to graph off of one dataset.

```{r m-LeBron-data, echo = TRUE}
...
The main takeaway with regards to this report is this: it is worth our time to include an interaction term between our categorical predictors $S_n$ and $D$ when dealing with $f$. Doing so, will help us model situations we saw in facets 1, 4, and 5.
...
\beta_\text{min_per_gm}X_\text{min_per_gm} +
...
test_data_predictions <- test_data_predictions %>% ungroup() %>%
  select(stage_of_game, gm_quarter, player_shooting, pts, min_per_gm, fg_pct, eff_fg_pct, ftm, fta, ft_pct, model_prob)

##print the first few lines of our predicted data
head(test_data_predictions, n = 10)
```

This concludes part 1 of building a holistic $m_p$. In this sub-section, we have built $m_p$ using only the statline of $p$ to give an adjusted free throw probability. In the next section, we will adjust the probability of $m_p$ by factoring in $S_n$ and $D$ and build a function $f$ to output $P(F)$. 

#### Analysis: Finding $f$

At this point, we have accounted for our numerical predictors. What we need to do is provide our model context with our categorical variables $S_n$ and $D$. 
