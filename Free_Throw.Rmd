...
The output shown above numerically shows $P(F)$ across the models $n$ (`ft_pct`), $m_p$ (`model_prob`), and $m$ (`model_prob_new`).  You can see that one noticeable difference is that each player has a different $P(F)$ under $m$ but shares the same $P(F)$ under $n$ and $m_p$ regardless of $S_n$. Also notice that for the first line of output shown above, $p = \text{Joel Embiid}$ has the same $P(F)$ under $m_p$ and $m$. Recall that $m$ only witnessed significance for certain $s \in S_n$ which we collectively called $s_n$. $s = 1.2$ is not included in $s_n$, so $m$ deffered to the $P(F)$ calculated by $m_p$. 


Now, one item remains, creating $f$. Again, we stated that a paramater of $f$ is $D$. But now, this is not the case. $f$ only needs two parameters: $p$ and $s \in S_n$
