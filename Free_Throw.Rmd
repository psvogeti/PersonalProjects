...
-   The key variables from the *Player Shot Statistics* data set we will use for this excursion:
    ...
    -   `eff_fg_pct` "effective field goal percentage; the formula is (FG + 0.5 * 3P) / FGA. This statistic adjusts for the fact that a 3-point field goal is worth one more point than a 2-point field goal"[^3]
    ...
    -   `ft_rate`the ratio between  `fta` and `fga` and can be viewed as the rate at which the `player` visits the free throw line based on shots taken [^4]
    
    [^3]: <https://www.basketball-reference.com/about/glossary.html>
    [^4]: <https://www.nbastuffer.com/analytics101/free-throw-rate/>
...
$$
F \in S_n \mid n = \{1.1, 1.2, 1.3, 1.4, 2.1, ... , 4.4\}
$$
...
Here we will clean up our mentioned data sets and add and remove certain variables as needed. Any significant changes are reported at the bootom of the clean up section.
