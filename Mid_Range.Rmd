...
##get the shots taken for the 2022-2023 NBA season
...
`locX` and `locY` are the standard media coordinates used by the NBA teams to locate where a player is with reference to the basketball court. The caveat to this system is that these coordinates represent an NBA-regulation half-court plane. This coordinate system, for our reference, will be referred to as $N$. 

For reference, let $x_n$ represent `locX` and $y_n$ represent `locY` such that a player's position $p_n$ on the half-court at any given moment is $p_n = (x_n, y_n)$. The origin of this system is directly underneath the basket. We define this origin as $o_n$ with $o_n = (0, 0)$. Again, all $p_n$ exist on this court $N$

Let's visualize this through some plotting. We do this in a rather fun way: we will create a function called `graph_player_shots()` that will take in a `player`, `team` and `reg_season` to ensure that we identify the right player for the right season. 

```{r plotting-on-N, echo=T}
graph_player_shots <- function(player_name, team, reg_season) {
  
  ##get the shot data of the player and the season
  player_shots_data <- all_shots_data %>% 
    filter((player_shooting == player_name) & (season == reg_season) & (player_team == team))
...
graph_player_shots(player_name = "Lowry", team = "TOR", reg_season = "2016-2017")

##graph Brunson's shots in the 2020-2021 NBA regular season
graph_player_shots(player_name = "Brunson", team = "DAL", reg_season = "2020-2021")

##graph Lillard's shots in the 2021-2022 NBA regular season
graph_player_shots(player_name = "Lillard", team = "POR", reg_season = "2021-2022")
...
Before we leave this section, let's visualize the numeric bounds of $N$. We will do this by picking one season, the 2022-2023 regular season, and examining what the shot spread looks like. 
...
ggplot() + 
  ...
  ggtitle("Defining the NBA Location Data Coordinate System", subtitle = "2022-2023 NBA Regular Season Shot Distribution") +
  theme_dark()
```
...
Since we have $o_n = o_r = `r origin_text[1]`$, all we need to do is apply a set of proportions to scale down our $p_n$ to $p_r$. These scaled values will then be interpreted as our possible $p$ coordinates for our final coordinate system $C$. 
...
$K = \frac{R}{N}$ directional proportions $k_x = \frac{x_r}{x_n}$ and $k_y = \frac{y_r}{y_n}$ 
This allows us to scale down our $p_n$ to $p_r$. This means that we can use our `locX` ($x_n$) and `locY` ($y_n$) data, use these proportioned coordinates on our new, final court that is very very similar to the $R$ court, $C$. 

For $(x_n, y_n) = p_n \in N$  

Coordinate system $C$ is the result of shrinking $N$'s bounds to agree with $R$'s bounds through 
proportion $K = \{k_x, k_y\}$

$$
C = \{p = (x, y) \mid x = x_n*k_x, y = y_n*k_y\}
$$
For $k_x = \frac{x_r}{x_n}$ and $k_y = \frac{y_r}{y_n}$ 
For $k_x, k_y \in K$
...
Here is the crux of our data cleanup: classifying our shots. To justify $C$, let's create a function that is very similar to `graph_player_shots()`, with the only change being we use `c.x` and `c.y` instead of `locX` and `locY`. We call this function `graph_standardized_shots()`

```{r graph-standardized-shots, echo=T}
graph_standardized_shots <- function(player_name, team, reg_season) {
...
##graph Lowry's shots in the 2016-2017 NBA regular season
graph_standardized_shots(player_name = "Lowry", team = "TOR", reg_season = "2016-2017")

##graph Brunson's shots in the 2020-2021 nba regular season
graph_standardized_shots(player_name = "Brunson", team = "DAL", reg_season = "2020-2021")

##graph Lillard's shots in the 2021-2022 nba regular season
graph_standardized_shots(player_name = "Lillard", team = "POR", reg_season = "2021-2022")
```
...
get_shot_distance <- function(x_coord, y_coord) {
  ...
  ## (x1, y1) = (0, 0) (shot distance from hoop and hoop is origin)
...
final_shots_data <- all_shots_data %>%
  ...
  select(season, gm_quarter, stage_of_game, de, player_team, player_shooting, c.x, c.y, shot_type, shot_result, shot_distance)
```
...
graph_classified_shots <- function(player_name, team, reg_season) {
...
##graph Lowry's shots in the 2016-2017 NBA regular season
graph_classified_shots(player_name = "Lowry", team = "TOR", reg_season = "2016-2017")

##graph Brunson's shots in the 2020-2021 nba regular season
graph_classified_shots(player_name = "Brunson", team = "DAL", reg_season = "2020-2021")

##graph Lillard's shots in the 2021-2022 nba regular season
graph_classified_shots(player_name = "Lillard", team = "POR", reg_season = "2021-2022")
```
...
```{r accuracy-plot}
graph_player_accuracies <- function(player_name, team) {
