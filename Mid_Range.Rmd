...
ggplot() + 
  ...
  ggtitle("Defining the NBA Location Data Coordinate System", subtitle = "2021-2022 NBA Regular Season") +
  theme_dark()
```
...
FINALLY, we can classify each shot. 
...
It is clear then that we must define the `"PAINT"` region. Let's call this region $P$, following the same exact structure as $C$, only that $P \subseteq C$: 
...
\}
$$  
The setup is complete. Let's classify our shots!
...
## use the function and the shot description to classify our shots
final_shots_data <- all_shots_data %>%
  ...
  select(season, gm_quarter, stage_of_game, de, player_shooting, c.x, c.y, shot_type, shot_result)
```

Let's test our classification system. We will pick a random player
```{r graph-classfied-shots}
